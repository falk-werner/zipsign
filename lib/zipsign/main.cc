...
namespace
{
    int sign(Arguments const & args, std::ostream & out, std::ostream & err)
    {
        (void) out;
    ...
    int verify(Arguments const & args, std::ostream & out, std::ostream & err)
    {
        auto const & filename = args.get('f');
        auto const & cert_files = args.getList('c');
        ...
        try
        {
            ...
            result = verifier.verify(filename, keyring_path, err, is_verbose, is_self_signed);
            ...
            switch (result) {
                ...
                case Verifier::BadMissingSignature:
                    out << "INVALID_MISSING_SIGNATURE" << std::endl;
                    break;
                case Verifier::BadInvalidCertificateChain:
                    out << "INVALID_CERTIFICATE_CHAIN" << std::endl;
                    break;
                case Verifier::BadInvalidSignature:
                    out << "INVALID_SIGNATURE" << std::endl;
                    break;
                case Verifier::Bad:
                    // fall-through
                default:
...
int main(int argc, char* argv[], std::ostream & out, std::ostream & err) noexcept
{
    try {
        ...
        return app.run(argc, argv, out, err);
    }
    catch (std::exception const & ex) {
        err << "error: " << ex.what() << std::endl;
        return EXIT_FAILURE;
